...
" js, ts, css , HTML, Markdown
au BufReadPost *.mak set syntax=html
au BufNewFile,BufRead *.md,*.js,*.ts,*.json,*.html,*.svg,*.mako,*.css,*.vue,*.svelte
    \ setlocal tabstop=2 |
    \ setlocal softtabstop=2 |
    \ setlocal shiftwidth=2 |
    \ setlocal textwidth=79 |
...
" Source - https://stackoverflow.com/a/37889460
" Posted by Christian Long, modified by community. See post 'Timeline' for change history
...
" autocmd FileType python setlocal indentkeys-=<:>
" autocmd FileType python setlocal indentkeys-=:
autocmd FileType * set fo-=t fo-=c fo-=r fo-=o fo-=q fo-=l
autocmd BufNewFile,BufRead * set fo-=t fo-=c fo-=r fo-=o fo-=q fo-=l 
" fo-=/
...
" project specific
set exrc      " Allows Vim to search for a local .vimrc file in the current directory
set secure    " Runs the local .vimrc in 'secure' mode, limiting potentially dangerous commands (like shell commands)
...
fun! TrimWhitespace()
    ...
    call winrestview(l:save)
endfun
autocmd BufWritePre *.c,*.ino,*.cpp,*.h,*.in,*.py call TrimWhitespace()
